...
public class RegisterCommands extends ListenerAdapter {
    ...
    public static void registerCommand() {
        Main.getDiscord().getBot().upsertCommand(CMD_Leave_s, "Lässt den Bot den Server verlassen").queue();
        Main.getDiscord().getBot().upsertCommand(CMD_Server_Owner_s, "Nennt den Server Inhaber").queue();
        Main.getDiscord().getBot().upsertCommand(CMD_Join_s, "Der Bot sendet dir einen link um ihn einzuladen").queue();
        Main.getDiscord().getBot().upsertCommand(CMD_Invite_s, "Du wirst eingeladen auf unseren Discord").queue();
        Main.getDiscord().getBot().upsertCommand(CMD_Join_Channel_s, "Der Bot betritt deinen Voice Channel").queue();
        Main.getDiscord().getBot().upsertCommand(CMD_Leave_Channel_s, "Der Bot verlässt deinen Voice Channel").queue();
        Main.getDiscord().getBot().upsertCommand(CMD_Help_s, "Zeigt eine Liste von möglichen Befehlen").queue();
        Main.getDiscord().getBot().upsertCommand(CMD_Birthday_s, "Gratuliere einem anderen User").addOption(OptionType.USER, "user", "Fügt einen anderen User hinzu", true).addOption(OptionType.BOOLEAN, "private", "Möchtest du dem User die Glückwünsche privat zukommen lassen?").queue();
        Main.getDiscord().getBot().upsertCommand(CMD_Bot_Owner_s, "Zeigt dir den Bot Programmierer").queue();
        Main.getDiscord().getBot().upsertCommand(CMD_Error_report_s, "Reporte einen Bot fehler").addOption(OptionType.STRING, "error", "Schildere hier deinen gefundenen Bot Fehler", true).queue();
        Main.getDiscord().getBot().upsertCommand(CMD_Ping_s, "Antwortet mit Pong").queue();
        Main.getDiscord().getBot().upsertCommand(CMD_Donate_s, "Zeigt dir eine Spende möglichkeit").queue();

        Main.getDiscord().getBot().upsertCommand(CMD_Server_Stats_s, "Zeigt dir die Stats des Servers").queue();
        Main.getDiscord().getBot().upsertCommand(CMD_Bot_Stats_s, "Zeigt dir die Stats des Bots").queue();
        Main.getDiscord().getBot().upsertCommand(CMD_Donate_s, "Zeigt dir eine Spende möglichkeit").queue();

        Main.getDiscord().getBot().upsertCommand(CMD_Ban_s, "Bannt einen bestimmten Spieler").addOption(OptionType.USER, "user", "Füge einen Benutzer hinzu", true).addOption(OptionType.INTEGER, "time", "Gib die Ban Dauer in Tagen an um den User zu bannen", true).addOption(OptionType.STRING, "reason", "Begründe deinen Ban", true).queue();
        Main.getDiscord().getBot().upsertCommand(CMD_Kick_s, "Kickt einen bestimmten Spieler").addOption(OptionType.USER, "user", "Füge einen Benutzer hinzu", true).addOption(OptionType.STRING, "reason", "Begründe deinen Kick", true).queue();
        Main.getDiscord().getBot().upsertCommand(CMD_TimeOut_s, "Timeoutet einen bestimmten Spieler").addOption(OptionType.USER, "user", "Füge einen Benutzer hinzu", true).addOption(OptionType.STRING, "time", "Gib die Timeout Dauer in Tagen an um den User zu timeouten. (In Minuten)", true).queue();

        Main.getDiscord().getBot().upsertCommand(CMD_Stop_s, "Stoppt den Bot").queue();
        Main.getDiscord().getBot().upsertCommand(CMD_Activity_s, "Ändert die Aktivität des Bots").addOption(OptionType.STRING, "type", "playing watching competing streaming listening custom", true).addOption(OptionType.STRING, "activity", "Hier kommt die Aktivität hin", true).addOption(OptionType.STRING, "url", "Füge deiner Option eine URL hinzu").queue();
